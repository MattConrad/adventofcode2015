...
func main() {
    reBs1 := regexp.MustCompile(`\\\\|\\"`)
    reHex := regexp.MustCompile(`\\x[a-f0-9][a-f0-9]`)
    reDq := regexp.MustCompile(`"`)
    reBs2 := regexp.MustCompile(`\\`)

    lines := getInput()
    countPart1 := 0
    countPart2 := 0
    //we can just count differences, we don't need to count code/encoded
    for _, line := range lines {
        // part 1: there are always two doublequotes per line, each backslash match (expr 1) is a diff of one, and each hex match is a diff of 3.
        countPart1 += 2 + len(reBs1.FindAllString(line, -1)) + (3 * len(reHex.FindAllString(line, -1)))
        // part 2: counting up, 2 NEW dqs per line, count dqs (including initial start/end dqs) as 1, count backslashes (expr 2) as 1
        countPart2 += 2 + len(reDq.FindAllString(line, -1)) + len(reBs2.FindAllString(line, -1))
    }

    fmt.Println("total part1 differences: ", countPart1)
    fmt.Println("total part2 differences: ", countPart2)
}

func getInput() []string {
	input := `"sjdivfriyaaqa\xd2v\"k\"mpcu\"yyu\"en"
"vcqc"
