...
func getBestSue() map[string]int {
...
func getCurrentSue(line string) (string, map[string]int) {
	sue := make(map[string]int)
	suename := ""
	dictparts := strings.Split(line, ", ")
	for i, part := range dictparts {
		//cough cough this isn't exactly crystal clear, perhaps I should have used a regex instead.
		subparts := strings.Split(part, ": ")
		if i == 0 {
			suename = subparts[0]
			sue[subparts[1]] = mustAtoi(subparts[2])
		} else {
			sue[subparts[0]] = mustAtoi(subparts[1])
		}
	}
...
func checkSue1(bestSue map[string]int, sue map[string]int) bool {
	rightsue := true
	for k, v := range sue {
		if bestSue[k] != v {
			rightsue = false
...
func checkSue2(bestSue map[string]int, sue map[string]int) bool {
	rightsue := true
	for k, v := range sue {
        //i found the text confusing, got the ranges backward first time.
		if k == "cats" || k == "trees" {
			if bestSue[k] >= v {
				rightsue = false
			}
...
func main() {
	...
	for _, line := range getInput() {
		suesname, sue := getCurrentSue(line)

		if checkSue1(bestSue, sue) {
			bestSues1Name = suesname
		}

		if checkSue2(bestSue, sue) {
			bestSues2Name = suesname
		}
	}

	fmt.Println(bestSues1Name)
	fmt.Println(bestSues2Name)
}

func getInput() []string {
	input := `Sue 1: cars: 9, akitas: 3, goldfish: 0
