...
var lights = make(map[light]bool)
var brights = make(map[light]int)

//processAction sets the value of points in the rectangle x1,y1/x2,y2 according to action toggle, on, or off.
func processAction(action string, x1 int, y1 int, x2 int, y2 int) {
	for i := x1; i <= x2; i++ {
		for j := y1; j <= y2; j++ {
			l := light{x: i, y: j}
			if action == "on" {
...
//processBright sets the brightness of points in the rectangle x1,y1/x2,y2 according to action toggle, on, or off. toggle has weird def, be careful.
func processBright(action string, x1 int, y1 int, x2 int, y2 int) {
	for i := x1; i <= x2; i++ {
		for j := y1; j <= y2; j++ {
			l := light{x: i, y: j}
			if action == "on" {
				brights[l]++
			} else if action == "off" && brights[l] > 0 {
				brights[l]--
			} else if action == "toggle" {
				brights[l] += 2
			}
...
//getIntCoords accepts a string containing a pair of coordinates separated by a comma, and returns a numeric x and y from that string.
func getIntCoords(coords string) (int, int) {
	parts := strings.Split(coords, ",")
	x, ok := strconv.Atoi(parts[0])
	y, ok2 := strconv.Atoi(parts[1])

	//i don't quite grok Go yet.
	if ok != nil || ok2 != nil {
...
//getInstructions takes a line and returns a tuple of action, x1, y1, x2, y2. action is "toggle", "off", or "on".
func getInstructions(line string) (string, int, int, int, int) {
	...
	if parts[0] == "toggle" {
		action = "toggle"
		x1, y1 = getIntCoords(parts[1])
		x2, y2 = getIntCoords(parts[3])
	} else {
		action = parts[1]
		x1, y1 = getIntCoords(parts[2])
		x2, y2 = getIntCoords(parts[4])
	}
...
func main() {
	...
	for _, line := range lines {
		action, x1, y1, x2, y2 := getInstructions(line)
		processAction(action, x1, y1, x2, y2)
		processBright(action, x1, y1, x2, y2)
	}

	count := countLights()
	count2 := countBrights()

	fmt.Println("lights lit: ", count)
	fmt.Println("brightness: ", count2)
}
